...
document.addEventListener("DOMContentLoaded", function(event){
		...
		var currentYr = theDate.getFullYear() ;
		var currentMon = theDate.getMonth();
		...
		function renderCalendar(targetElem){
				...
				nextMonthSpan.addEventListener('click', function(e){
					var key = e.which||e.keyCode;
					if(key == 39){
					 goToMonth(currentDate, true); // Go To Previous Month	
					}
				});
				...
				/*var text_yr = document.createElement("input");
				text_yr.setAttribute("id","text_yr"); 
				text_yr.setAttribute("placeholder","Enter year"); 
				text_yr.addEventListener('keyup', function(event){

					var result = isNumberKey(event);
				...
				document.getElementById("user_yr").focus();
		...
		function goToMonth(currentDate, direction) {
			var yr_val = document.getElementById("user_yr").value;
			var valid_y = currentYr-parseInt(yr_val);
			var valid_date = new Date(valid_y,currentMon,currentDay);
			if(yr_val != ''){
				if (direction == false){
					var mon=theDate.getMonth();
					theDate = new Date(valid_y, currentMon,currentDay);
			    } else{ 
				    theDate = new Date(currentYr, currentMon, currentDay);
			    }
			}
			else{
				if (direction == false){
		...
		function isNumberKey(evt)
      {
		...
		function test(){
			var yr_val = document.getElementById("text_yr").value;
			console.log("test: "+yr_val);
			theDate = new Date(theDate.getFullYear()+parseInt(yr_val), theDate.getMonth(), currentDate.theDay);
			return renderCalendar("calendarThis");
		}
